...
public:
  ...
  void crateID_100() {
    EQMSG("100 MSPS extract crate ID", uint32_t(3), hit100.getCrateID());
  }

  /** @brief Check the slot ID value. */
  void slotID_100() {
    EQMSG("100 MSPS extract slot ID", uint32_t(2), hit100.getSlotID());
  }

  /** @brief Check the channel ID value. */
  void chanID_100() {
    EQMSG("100 MSPS extract channel ID", uint32_t(1), hit100.getChannelID());
  }

  /** @brief Check the number of header words. */
  void headerLength_100() {
    EQMSG("100 MSPS extract header length", uint32_t(18),
          hit100.getChannelHeaderLength());
  }

  /** @brief Check the event length in 32-bit words. */
  void eventLength_100() {
    EQMSG("100 MSPS extract event length", uint32_t(22),
          hit100.getChannelLength());
  }
  ...
  void msps_100() {
    EQMSG("100 MSPS extract adc frequency", uint32_t(100), hit100.getModMSPS());
  }

  /** @brief Check lower 32 bits of 48 bit timestamp. */
  void timeLow_100() {
    EQMSG("100 MSPS extract time low", uint32_t(63111), hit100.getTimeLow());
  }

  /** @brief Check upper 16 bits of 48 bit timestamp. */
  void timeHigh_100() {
    EQMSG("100 MSPS extract time high", uint32_t(10), hit100.getTimeHigh());
  }

  /** @brief Check reconstructed coarse timestamp (in ns). */
  void coarseTime_100() {
    EQMSG("100 MSPS coarse time", uint64_t(0x000a0000f687) * 10,
          hit100.getCoarseTime());
  }

  /** @brief Check corrected full timestamp (in ns). */
  void time_100() {
    // The magic number is reconstructed TS for 100 MSPS:
    ASSERTMSG("100 MSPS full time",
              std::abs(hit100.getTime() - 429497360711.601257) < 0.000001);
  }

  /** @brief Check that we can parse the CFD fail word. */
  void cfdFail_100() {
    EQMSG("100 MSPS compute CFD fail bit", uint32_t(1), hit100.getCFDFailBit());
  }
  ...
  void coarseTime_250() {
    EQMSG("250 MSPS compute coarse time", uint64_t(0x000a0000f687) * 8,
          hit250.getCoarseTime());
  }

  /** @brief Read computed time from 250 MSPS module. */
  void time_250() {
    // The magic number is reconstructed TS for 250 MSPS:
    ASSERTMSG("250 MSPS compute time",
  ...
  void cfdFail_250() {
    ...
    EQMSG("250 MSPS compute CFD fail bit", uint32_t(0), hit250.getCFDFailBit());
  }
  ...
  void coarseTime_500() {
    EQMSG("500 MSPS compute coarse time", uint64_t(0x000a0000f687) * 10,
          hit500.getCoarseTime());
  }

  /** @brief Read computed time from 500 MSPS module. */
  void time_500() {
    // The magic number is reconstructed TS for 500 MSPS:
    ASSERTMSG("500 MSPS compute time",
  ...
  void cfdFail_500_0() {
    ...
    EQMSG("500 MSPS compute CFD fail bit", uint32_t(0), hit500.getCFDFailBit());
  }
  ...
  void cfdFail_500_1() {
    ...
    std::vector<uint32_t> data = {0x0000000c, 0x0f0e01f4, 0x00084321,
                                  0x0000f687, 0xf47f000a, 0x000008b3};
    ...
    EQMSG("500 MSPS compute CFD fail bit", uint32_t(1), hit.getCFDFailBit());
  }
